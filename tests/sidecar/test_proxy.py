...
def _build_app(upstream_app, tmp_path):
...
def test_health_check_path_falls_through_to_upstream_health(dummy_upstream_app, tmp_path):
    app = _build_app(dummy_upstream_app, tmp_path)
...
def test_execute_body_is_forwarded_and_echoed(dummy_upstream_app, tmp_path):
    app = _build_app(dummy_upstream_app, tmp_path)
    with TestClient(app) as client:
...
def test_query_params_and_headers_are_forwarded(dummy_upstream_app, tmp_path):
    app = _build_app(dummy_upstream_app, tmp_path)
    with TestClient(app) as client:
...
def test_upstream_error_status_relayed(dummy_upstream_app, tmp_path):
    app = _build_app(dummy_upstream_app, tmp_path)
    with TestClient(app) as client:
