...
This was verified against a real cluster (`kind` + the `hashicorp/consul` Helm chart, `syncCatalog.enabled=true`, `connectInject.enabled=false`) — see `k8s/example/` for the exact, working manifests. Three things turned out to matter in practice that weren't obvious from the documentation alone:

- **The `consul.hashicorp.com/service-name` annotation on the Service is required, not optional.** Without it, Service Sync registers the service as `<name>-<namespace>` (e.g. `magic-echo-a-default`) rather than the plain name — controlled by the chart's `syncCatalog.addK8SNamespaceSuffix`, which defaults to `true`. Since the AP graph node's slugified `properties.name` must match the Consul service name exactly, every operator's Service needs this annotation set to its intended slug (see `k8s/example/service.yaml`).
- **Health status is a single synthetic "Kubernetes Readiness Check"**, not a Consul-executed HTTP check — Service Sync just reflects the Pod's own `Ready` condition into Consul as one check. This confirms the readiness-probe wiring above is sufficient and is the *only* health signal that reaches Consul this way (there's no separate `interval`/`timeout`/`deregister_critical_service_after` to configure, unlike the Compose Consul-agent path).
- **Service Sync cannot set custom `Service.Meta` fields (no `version`, no arbitrary metadata).** Its `Meta` is fixed, auto-generated Kubernetes bookkeeping (`external-k8s-ns`, `external-k8s-ref-name`, etc.) — there is no Helm value or annotation to inject a `version` key. This means **AP graph nodes with a pinned `properties.version` will not resolve correctly against a Service-Sync-registered Kubernetes operator** — `ConsulRegistryClient`'s `Service.Meta.version == "<version>"` filter matches nothing, since the field is simply absent, and the operator would appear unresolvable rather than resolved-to-the-wrong-version. Version pinning on Kubernetes needs a different mechanism (e.g. deploying distinct Consul service names per version and picking one via the AP graph node's `properties.name` instead of `properties.version`) — there's no drop-in equivalent to the Compose path's free-form `Meta` today.
